...
static bool max30102_rd8(uint8_t reg, uint8_t *val) {
    if (i2c_write_blocking(I2C_MAX_PORT, MAX30102_ADDR, &reg, 1, true) != 1) return false;
...
static bool max_init_hw(void) {
    max30102_wr8(0x09, 0x40); sleep_ms(10); // Reset
    max30102_wr8(0x09, 0x00); sleep_ms(10);
    
    if (!max30102_wr8(0x02, 0x00)) return false; // Int enable
    if (!max30102_wr8(0x04, 0)) return false;    // FIFO Wr Ptr
    if (!max30102_wr8(0x05, 0)) return false;    // Ovf Counter
    if (!max30102_wr8(0x06, 0)) return false;    // FIFO Rd Ptr
    if (!max30102_wr8(0x08, 0x4F)) return false; // FIFO config: avg 4, rollover enabled
    if (!max30102_wr8(0x0A, 0x27)) return false; // Mode: SpO2, 100Hz, 18bit
    if (!max30102_wr8(0x0C, 0x24)) return false; // LED Red Pulse Amp (Ajustado levemente)
    if (!max30102_wr8(0x0D, 0x24)) return false; // LED IR Pulse Amp (Ajustado levemente)
    if (!max30102_wr8(0x09, 0x03)) return false; // Mode control: Multi-LED
    return true;
}
...
void check_fall_logic(FallDetector_t *fd, VECT_3D *accel) {
    ...
    if (accel_total < FREE_FALL_THRESHOLD && !fd->free_fall_detected) {
        ...
        snprintf(msg.payload, sizeof(msg.payload), "{\"status\": \"queda_livre\", \"g\": %.2f}", accel_total);
    ...
    if (fd->free_fall_detected && accel_total > IMPACT_THRESHOLD) {
        snprintf(msg.topic, sizeof(msg.topic), "%s", MQTT_TOPIC_FALL);
        snprintf(msg.payload, sizeof(msg.payload), "{\"status\": \"impacto\", \"g\": %.2f}", accel_total);
        xQueueSend(mqttQueue, &msg, 0);
    ...
    if (fd->impact_detected) {
        ...
        if (time_since_fall > 500 && time_since_fall < FALL_TIMEOUT_MS) {
            if (accel_total > 0.8f && accel_total < 1.2f) { // Estabilizado
                snprintf(msg.topic, sizeof(msg.topic), "%s", MQTT_TOPIC_FALL);
...
float process_pulse_logic(PulseState *s, uint32_t red, uint32_t ir, int idx) {
    ...
    if (s->finger_on) {
        ...
        if (ac_ir > 300 && s->acs_prev1 <= 300) {
             ...
             if(rr > 0.35 && rr < 1.8) { // 33 a 170 BPM
                 ...
                 if (s->bpm_idx == 0) s->buffer_filled = true;
        ...
        // Limita o teto
        if (calculated_spo2 > 100.0f) calculated_spo2 = 100.0f;
            ...
            for (int i=0; i<num_samples; i++) {
                 ...
                 if (st.finger_on && (xTaskGetTickCount() - last_pub > pdMS_TO_TICKS(2000))) {
                      ...
                      if (st.bpm > 40.0f) {
                          ...
                          snprintf(msg.payload, sizeof(msg.payload), "{\"bpm\": %.0f, \"spo2\": %.0f}", st.bpm, spo2);
                          xQueueSend(mqttQueue, &msg, 0);
                          last_pub = xTaskGetTickCount();
                          printf("HR Enviado -> BPM: %.1f SpO2: %.1f\n", st.bpm, spo2);
                      }
                 }
            }
        }
