...
// --- MPU6050 Helpers ---
bool mpu6050_init_hw(MPU6050_t *mpu, i2c_inst_t *i2c, uint8_t addr)
{
    printf("[MPU] Inicializando MPU6050 no endereço 0x%02X...\n", addr);
    
    mpu->i2c = i2c;
    mpu->addr = addr;
    
    // Teste de comunicação I2C
    printf("[MPU] Testando comunicação I2C...\n");
    uint8_t test_byte;
    int ret = i2c_read_blocking(mpu->i2c, mpu->addr, &test_byte, 1, false);
    if (ret < 0) {
        printf("[MPU] ERRO: Falha na comunicação I2C! Código: %d\n", ret);
        return false;
    }
    
    // Wake up MPU6050
    printf("[MPU] Acordando MPU6050...\n");
    uint8_t buffer[2] = {0x6B, 0x00}; // PWR_MGMT_1 register
    ret = i2c_write_blocking(mpu->i2c, mpu->addr, buffer, 2, false);
    if (ret != 2) {
        printf("[MPU] ERRO: Falha ao escrever no registrador 0x6B! Retorno: %d\n", ret);
        return false;
    }
    
    // Configurar acelerômetro (0x1C = ACCEL_CONFIG)
    printf("[MPU] Configurando acelerômetro...\n");
    buffer[0] = 0x1C; // ACCEL_CONFIG register
    buffer[1] = 0x00; // ±2g (padrão)
    ret = i2c_write_blocking(mpu->i2c, mpu->addr, buffer, 2, false);
    if (ret != 2) {
        printf("[MPU] ERRO: Falha ao configurar acelerômetro! Retorno: %d\n", ret);
        return false;
    }
    
    printf("[MPU] MPU6050 inicializado com sucesso!\n");
    return true;
}

bool mpu6050_get_accel(MPU6050_t *mpu, VECT_3D *vect)
{
    uint8_t buffer[6];
    ...
    if (ret != 1) {
        printf("[MPU] ERRO: Falha ao escrever endereço do registrador! Retorno: %d\n", ret);
        return false;
    }
    
    // Ler 6 bytes de dados
    ret = i2c_read_blocking(mpu->i2c, mpu->addr, buffer, 6, false);
    if (ret != 6) {
        printf("[MPU] ERRO: Falha ao ler dados do acelerômetro! Retorno: %d\n", ret);
        return false;
    }
...
void check_fall_logic(FallDetector_t *fd, VECT_3D *accel)
{
    ...
    // Log dos valores do acelerômetro (reduzir frequência para não sobrecarregar)
    static uint32_t last_log_time = 0;
    uint32_t current_time = to_ms_since_boot(get_absolute_time());
    
    if (current_time - last_log_time > 1000) { // Log a cada 1 segundo
        printf("[MPU] Aceleração: X=%.2fg, Y=%.2fg, Z=%.2fg, Total=%.2fg\n", 
               accel->x, accel->y, accel->z, accel_total);
    ...
    if (accel_total < FREE_FALL_THRESHOLD && !fd->free_fall_detected)
    {
        printf("[MPU] DETECTADO: Queda Livre! acel_total=%.2fg (limite=%.2fg)\n", 
               accel_total, FREE_FALL_THRESHOLD);
    ...
    if (fd->free_fall_detected && accel_total > IMPACT_THRESHOLD)
    {
        printf("[MPU] DETECTADO: Impacto! acel_total=%.2fg (limite=%.2fg)\n", 
               accel_total, IMPACT_THRESHOLD);
    ...
    if (fd->impact_detected)
    {
        ...
        if (time_since_fall > 500 && time_since_fall < FALL_TIMEOUT_MS)
        {
            if (accel_total > 0.8f && accel_total < 1.2f)
            { // Estabilizado
                printf("[MPU] CONFIRMADO: Queda detectada! Aceleração estabilizada em %.2fg\n", accel_total);
        ...
        {
            printf("[MPU] Timeout: resetando detector de queda\n");
            memset(fd, 0, sizeof(FallDetector_t)); // Timeout
            fd->prev_accel_total = 1.0f;
        }
    }
...
void vTaskMPU(void *pvParameters)
{
    ...
    printf("[MPU] Iniciando tarefa MPU...\n");
    
    // Inicializar MPU6050
    if (!mpu6050_init_hw(&mpu, I2C_MPU_PORT, MPU6050_ADDR)) {
        printf("[MPU] ERRO CRÍTICO: Falha na inicialização do MPU6050!\n");
        vTaskDelete(NULL);
    }
    
    printf("[MPU] MPU6050 inicializado com sucesso na tarefa\n");
    ...
    {
        // Ler acelerômetro
        if (!mpu6050_get_accel(&mpu, &accel)) {
            read_errors++;
            printf("[MPU] ERRO: Falha ao ler acelerômetro (%d/%d)\n", read_errors, max_errors);
            
            if (read_errors >= max_errors) {
                printf("[MPU] ERRO CRÍTICO: Muitas falhas consecutivas!\n");
                // Tentar reinicializar
                if (!mpu6050_init_hw(&mpu, I2C_MPU_PORT, MPU6050_ADDR)) {
                    printf("[MPU] Não foi possível reinicializar o MPU6050\n");
                } else {
                    read_errors = 0;
                    printf("[MPU] MPU6050 reinicializado com sucesso\n");
                }
            }
        } else {
            read_errors = 0; // Reset contador de erros se leitura foi bem-sucedida
            check_fall_logic(&fd, &accel);
        }
...
void vTaskMAX(void *pvParameters)
{
    printf("[MAX] Iniciando tarefa MAX30102...\n");
    
    if (!max_init_hw())
    {
        printf("[MAX] ERRO: Falha na inicialização do MAX30102\n");
        vTaskDelete(NULL);
    }
    
    printf("[MAX] MAX30102 inicializado com sucesso\n");
            ...
            for (int i = 0; i < num_samples; i++)
            {
                ...
                if (st.finger_on && (xTaskGetTickCount() - last_pub > pdMS_TO_TICKS(2000)))
                {
                    ...
                    if (st.bpm > 40.0f)
                    {
                        ...
                        xQueueSend(mqttQueue, &msg, 0);
                        last_pub = xTaskGetTickCount();
                        printf("[MAX] HR Enviado: BPM=%.1f SpO2=%.1f\n", st.bpm, spo2);
                    }
                }
            }
        }
...
int main()
{
    ...
    sleep_ms(2000);
    printf("==========================================\n");
    printf("Sistema Iniciado - Versão Corrigida v2 com LOGS\n");
    printf("==========================================\n\n");

    printf("Inicializando I2C para MPU6050...\n");
    i2c_init(I2C_MPU_PORT, 400000);
    gpio_set_function(SCL_MPU_PIN, GPIO_FUNC_I2C);
    gpio_set_function(SDA_MPU_PIN, GPIO_FUNC_I2C);
    gpio_pull_up(SCL_MPU_PIN);
    gpio_pull_up(SDA_MPU_PIN);
    printf("I2C0 (MPU6050) inicializado: SCL=GPIO%d, SDA=GPIO%d\n", SCL_MPU_PIN, SDA_MPU_PIN);

    printf("Inicializando I2C para MAX30102...\n");
    i2c_init(I2C_MAX_PORT, 400000);
    gpio_set_function(SCL_MAX_PIN, GPIO_FUNC_I2C);
    gpio_set_function(SDA_MAX_PIN, GPIO_FUNC_I2C);
    gpio_pull_up(SCL_MAX_PIN);
    gpio_pull_up(SDA_MAX_PIN);
    printf("I2C1 (MAX30102) inicializado: SCL=GPIO%d, SDA=GPIO%d\n", SCL_MAX_PIN, SDA_MAX_PIN);

    printf("Criando fila MQTT...\n");
    mqttQueue = xQueueCreate(10, sizeof(MqttMsg_t));
    if (mqttQueue == NULL) {
        printf("ERRO: Falha ao criar fila MQTT!\n");
        while(1);
    }
    printf("Fila MQTT criada com sucesso\n");
    ...
    printf("ERRO: Scheduler FreeRTOS falhou!\n");
    while (1)
        ;
    return 0;
}
